...
public class Router {
    ...
    public static List<Long> shortestPath(GraphDB g,
        ...
        // TODO
        Long curr = 0L;
        //Vertex id to best known distance.
        HashMap<Long, Double> disTo = new HashMap<>();
        //Vertex id to best vertex id.
        HashMap<Long, Long> edgeTo = new HashMap<>();
        HashSet<Long> marked = new HashSet<>();
        long strt = g.closest(stlon, stlat);
        long dest = g.closest(destlon, destlat);
        PriorityQueue<Long> fringe = new PriorityQueue<>(
            (Long o1, Long o2)
                -> Double.compare((disTo.get(o1) + g.distance(o1, dest)),
                    disTo.get(o2) + g.distance(o2, dest)));

        fringe.add(strt);
        marked.add(strt);

        for (Long v: g.vertices()) {
            disTo.put(v, Double.MAX_VALUE);
        }
        disTo.put(strt, 0.0);
        edgeTo.put(strt, -0L);
        ...
        while (!fringe.isEmpty()) {
            ...
            for (Long n: g.adjacent(curr)) {

                double bestDis = disTo.get(curr) + g.distance(curr, n);

                if (bestDis < disTo.get(n) && !marked.contains(n)) {
