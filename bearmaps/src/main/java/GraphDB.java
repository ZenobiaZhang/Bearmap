...
public class GraphDB {
    ...
    //needs a graph-like structure to store the nodes
    HashMap<Long, Vertex> allVertexes;
    KdTree t;
    ...
    private void clean() {
        // TODO
        LinkedList<Vertex> toRemove = new LinkedList<>();
        for (Vertex v: allVertexes.values()) {
            if (v.getNeighborVertexes().isEmpty()) {
                toRemove.add(v);
            }
        }
        for (Vertex v: toRemove) {
            allVertexes.remove(v.getId());
        }
    }
    ...
    double lon(long v) {
        // TODO
        if (allVertexes.containsKey(v)) {
            return allVertexes.get(v).getLon();
        }
    ...
    double lat(long v) {
        // TODO
        if (allVertexes.containsKey(v)) {
            return allVertexes.get(v).getLat();
        }
        return 0.0;
    }
    ...
    Iterable<Long> adjacent(long v) {
        ...
        return allVertexes.get(v).getNeighborVertexes().keySet();
    }
    ...
    public long closest(double lon, double lat) {
        ...
        return t.nearestNeighbor(lon, lat).node.getId();

//        Vertex ref = new Vertex(0, -lon, -lat);
//        long minID = 0;
//        for (Vertex v: allVertexes.values()) {
//            if (v.distanceTo(lon, lat) < ref.distanceTo(lon, lat)) {
//                ref = v;
//                minID = v.getId();
//            }
//        }
//        return minID;
    ...
    //added 2 classed and corresponding methods,may or may not be all useful - MAX

    public void addVertext(long id, Vertex v) {
        allVertexes.put(id, v);
    }
    ...
    //stores the information of connection in the graph
    public class Way {
        Vertex start;
        Vertex end;
        double maxspeed;
        ...
        Way(Vertex s, Vertex e, long weight) {
            this.start = s;
            this.end = e;
            //this.weight = weight;
        }
    ...
    //creates a vertex that stores information of node.
    static class Vertex {
        long id;
        double lon;
        double lat;
        HashSet<Way> neighborEdges;
        HashMap<Long, Vertex> neighborVertexes;

        Vertex(long id, double lon, double lat) {
            neighborVertexes = new HashMap<>();
        ...
        public void addNeighborEdge(Way e) {
            neighborEdges.add(e);
        }

        private HashSet<Way> getNeighborEdges() {
            return this.neighborEdges;
        }

        public void addNeighborVertex(Vertex v) {
            neighborVertexes.put(v.getId(), v);
        }

        private HashMap<Long, Vertex> getNeighborVertexes() {
            return this.neighborVertexes;
        }

        public double distanceTo(double lonn, double latt) {
            double phi1 = Math.toRadians(latt);
            double phi2 = Math.toRadians(this.getLat());
            double dphi = Math.toRadians(latt - this.getLat());
            double dlambda = Math.toRadians(lonn - this.getLon());
